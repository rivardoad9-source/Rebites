...
/**
 * Re-kalkulasi paksa: jalankan ulang FIFO untuk semua baris lalu tulis balik
 * kolom turunan ke Google Sheets. Dipakai tombol "Sinkron" di dashboard dan
 * webhook Google Apps Script (onEdit/onChange).
 */
export async function POST(request: Request) {
  try {
    ...
    return ok({ recalculated: result, snapshot: await getSnapshot() });
  } catch (error) {
    return handleError(error);
  }
}
